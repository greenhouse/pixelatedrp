...
function OpenMenuGarage()
	...
	local elements = {
		{label = "Take out vehicle", value = 'list_vehicles'},
		{label = "List by brand", value = 'list_brands'},
		{label = "Retrieve vehicle from streets ("..Config.Price.."$)", value = 'return_vehicle'},
	}
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if(data.current.value == 'list_brands') then
				listVehicleBrand()
			end
...
function listVehicleBrand()
	local elements = {}
	local hash = {}
	ESX.TriggerServerCallback('eden_garage:getPlayerBrands', function(brands)

		for _,v in pairs(brands) do
			local brand = v.brands
			if (not hash[brand]) then
			print(brand)
			table.insert(elements, {label = brand , value = brand})
			hash[brand] = true
			end
		end
		...
		function(data, menu)
			if(data.current.value)then
				menu.close()
				ListVehiclesMenuBrand(data.current.value)
			end
		end,
		function(data, menu)
			menu.close()
...
function ListVehiclesMenuBrand(Cbrand)
	local elements = {}
	local options  = {}
	local vehiclesByBrand = {}

	ESX.TriggerServerCallback('eden_garage:getVehicles', function(vehicles)
		for _,v in pairs(vehicles) do
				local hashVehicule = v.vehicle.model
				local vehicleName = v.modelname
				local labelvehicle
				local brand = v.brand

				if(v.state)then
					if brand == Cbrand then
...
function SpawnVehicle(vehicle)
	...
	ESX.Game.SpawnVehicle(vehicle.model, {
		x = this_Garage.SpawnPoint.Pos.x + 3,
		y = this_Garage.SpawnPoint.Pos.y ,
		z = this_Garage.SpawnPoint.Pos.z + 1
	},this_Garage.SpawnPoint.Pos.h, function(callback_vehicle)
...
AddEventHandler('eden_garage:hasEnteredPark', function(zone)
	if zone == 'garage' then
		CurrentAction     = 'garage_park_action_menu'
		CurrentActionMsg  = "Press ~INPUT_PICKUP~ to put the car inside your garage"
		CurrentActionData = {}
	end
end)
...
AddEventHandler('eden_garage:hasExitedPark', function(zone)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)
...
function ReturnVehicleMenu()
	...
	ESX.TriggerServerCallback('eden_garage:getOutVehicles', function(vehicles)
		...
		for _,v in pairs(vehicles) do
    		...
    		labelvehicle = vehicleName..': retrieve'
...
Citizen.CreateThread(function()
	...
	while true do
		...
		-- Put car back
		for _,v in pairs(Config.Garages) do
			if(GetDistanceBetweenCoords(coords, v.DeletePoint.Pos.x, v.DeletePoint.Pos.y, v.DeletePoint.Pos.z, true) < v.DeletePoint.Size.x) then
				isInMarker  = true
				this_Garage = v
			end
		end
		...
		if isInMarker and not hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = true
			LastZone                = currentZone
			TriggerEvent('eden_garage:hasEnteredPark', currentZone)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('eden_garage:hasExitedpark', LastZone)
		end
		-- End put car back
	...
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and (GetGameTimer() - GUI.Time) > 150 then
				...
				if CurrentAction == 'garage_park_action_menu' then
					Parkcar()
				end

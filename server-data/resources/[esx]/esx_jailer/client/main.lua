...
RegisterNetEvent("esx_jailerEL:jail")
AddEventHandler("esx_jailerEL:jail", function(jailTime)
	if escape == true then
		return
	end
		...
		Citizen.CreateThread(function()
			...
			while jailTime > 0 and not unjail do
				...
				if jailTime % 120 == 0 then
					...
					TriggerServerEvent('esx_jailerEL:updateRemaining', jailTime)
				end
			...
			-- jail time served
			TriggerServerEvent('esx_jailerEL:unjailTime', -1)
			SetEntityCoords(sourcePed, Config.JailBlip.x, Config.JailBlip.y, Config.JailBlip.z)
			TriggerEvent('esx_policejob:unrestrain')
			TriggerEvent('esx_ambulancejob:revive', sourcePed)
			escape = false
...
RegisterNetEvent("esx_jailerEL:unjail")
AddEventHandler("esx_jailerEL:unjail", function(source)
	unjail = true
end)

AddEventHandler('playerSpawned', function(spawn)
	TriggerServerEvent('esx_jailerEL:checkjail')
end)

...
AddEventHandler('esx_phone:loaded', function(phoneNumber, contacts)
  ...
  SendNUIMessage({
    reloadPhone = true,
    phoneData   = PhoneData,
    accountBalance = accountBalanceData
  })
...
RegisterNetEvent('esx_phone:notiUser')
AddEventHandler('esx_phone:notiUser', function(mesgTitle,Nmessage)
    local mugshot, mugshotStr = ESX.Game.GetPedMugshot(GetPlayerPed(-1))
    ESX.ShowAdvancedNotification('Phone', mesgTitle, Nmessage , mugshotStr, 1)
...
AddEventHandler('esx_phone:onMessage', function(phoneNumber, message, position, anon, job, dispatchRequestId)
  ...
  if job == 'player' then
    ...
    TriggerEvent('esx_phone:notiUser', '~b~New message:~s~ %s' , message)
    TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 10, "definite", 1.0)
  else
    --ESX.ShowNotification('~b~' .. job .. ': ~s~' .. message)
    TriggerEvent('esx_phone:notiUser', job, '~s~'..message)
    TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 10, "definite", 1.0)
  end
...
AddEventHandler('esx_phone:stopDispatch', function(dispatchRequestId, playerName)
  ...
  if CurrentDispatchRequestId == dispatchRequestId and CurrentAction == 'dispatch' then
    ...
    TriggerEvent('esx_phone:notiUser','', playerName .. _U('taken_call'))
  end
...
RegisterNUICallback('setGPS', function(data)
  ...
  TriggerEvent('esx_phone:notiUser','', _U('gps_position'))
end)
...
RegisterNUICallback('send', function(data)
  ...
  TriggerEvent('esx_phone:notiUser','', _U('message_sent'))
  TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 10, "msgsend", 1.0)
...
Citizen.CreateThread(function()
  while true do
    ...
    if GUI.PhoneIsShowed then -- codes here: https://pastebin.com/guYd0ht4
      ...
      if IsControlPressed(0, Keys['Y']) and (GetGameTimer() - GUI.Time) > 150 then

        if not ESX.UI.Menu.IsOpen('phone', GetCurrentResourceName(), 'main') then
          ESX.UI.Menu.CloseAll()
          ESX.TriggerServerCallback('esx_phone:getBankAccountBalance', function(data)
            SendNUIMessage({
              accountBalance = data
            })
          end)
          ESX.TriggerServerCallback('esx_phone:getBankBalance', function(data2)
            SendNUIMessage({
              accountBank = data2
            })
          end)
...
Citizen.CreateThread(function()
  while true do
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlPressed(0,  Keys['E']) and (GetGameTimer() - GUI.Time) > 500 then

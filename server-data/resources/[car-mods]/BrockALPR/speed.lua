...
Citizen.CreateThread( function()
	...
	while true do
		Wait(0)
		if IsControlJustPressed(1, 128) and IsPedInAnyPoliceVehicle(GetPlayerPed(-1)) and GetLastInputMethod( 0 ) then
		...
		if IsControlJustPressed(1, 127) and IsPedInAnyPoliceVehicle(GetPlayerPed(-1)) and GetLastInputMethod( 0 ) then
		...
		if radar.shown  then
			if radar.freeze == false then
					...
					if IsEntityAVehicle(e) then
						...
						local fvspeed = math.ceil(GetEntitySpeed(e) * 3.6)
					...
					if IsEntityAVehicle(j) then
						...
						local bvspeed = math.ceil(GetEntitySpeed(e) * 3.6)

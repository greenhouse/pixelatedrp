...
ESX.RegisterServerCallback('esx_phone:getBankAccountBalance', function(source, cb)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local steamid = xPlayer.identifier

	MySQL.Async.fetchAll(
		"SELECT * FROM addon_account_data WHERE owner = @identifier AND account_name = @accountName ",
		{
			['@identifier'] = steamid,
      ['@accountName'] = 'bank_savings'
		},
		function(result)
			local data = ''
			for i=1, #result, 1 do
...
ESX.RegisterServerCallback('esx_phone:getBankBalance', function(source, cb)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local steamid = xPlayer.identifier

(function(){
	...
	let showBankAccount = function(){
		$('#bankaccount').addClass('active');
		isBackAOpen = true;
	}

	let hideBackAccount = function(){
	...
	let renderSpecialContacts = function(){
		...
		$('#phone .menu .home').html(
			...
			'<li class="phone-icon" id="phone-icon-bank">Bank</li>'
		);
	...
	$('#btn-head-back-bank').click(function() {
		hideBackAccount();
	});
	...
	window.onData = function(data){
		...
		if(data.accountBalance != 0){
			if(data.accountBalance === undefined){
				$('.bankbalance').text('You have no savings account, you can visit a banker to set one up');
			}else{
				$('.bankbalance').text('$ ' + data.accountBalance);
			}
		}

		if(data.accountBank != 0){
			if(data.accountBank === undefined){
				$('.bankmoney').text('You have no money');
			}else{
				$('.bankmoney').text('$ ' + data.accountBank);
			}
		}

...
local emplacement = {
{name="Money Wash", id=108, colour=75, x=1269.73, y=-1710.25, z=54.7715},
}
local options = {
    x = 0.1,
    y = 0.2,
    ...
    scale = 0.4,
    font = 0,
    menu_title = "Money wash", -- title menu // titre menu
...
Citizen.CreateThread(
	function()
	--X, Y, Z coords
		local x = 1122.1237792969
		local y = -3194.4958496094
		local z = -41.35
		while true do
			Citizen.Wait(0)
			...
			if (Vdist(playerPos.x, playerPos.y, playerPos.z, x, y, z) < 100.0) then
				...
				if (Vdist(playerPos.x, playerPos.y, playerPos.z, x, y, z) < 4.0) then
					DisplayHelpText('Press ~INPUT_CONTEXT~ to ~g~Wash~s~ your ~r~dirty money')
...
function BlanchirMenu()
   options.menu_subtitle = "Clean your money"
    ClearMenu()
	Menu.addButton("Wash", "Blanchir", -1)
		...
		if(amount == -1) then
			...
			if (GetOnscreenKeyboardResult()) then
				...
				if(res ~= nil and res ~= 0 and res <= maxDirty) then
                 ...
                 Notify("~r~You can not wash that big sum, it wont fit in the machine!")
				end
			end
		end
		if(amount ~= -1) then

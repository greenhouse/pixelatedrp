...
AddEventHandler('esx_carthief:delivered', function(value)
  ...
  totalPlayer  = total / 100 * 75
  totalAccount = total / 100 * 25

  TriggerEvent('esx_addonaccount:getSharedAccount', 'society_carthief', function(account)
    account.addMoney(totalAccount)
  end)

  xPlayer.addMoney(totalPlayer)

	TriggerClientEvent('esx:showNotification', xPlayer.source, Config.STRING_SOLD_CAR_VALUE .. totalAccount)
	TriggerClientEvent('esx:showNotification', xPlayer.source, Config.STRING_SOLD_CAR_VALUE2 .. totalPlayer)
end)

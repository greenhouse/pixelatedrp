		...
		while true do
			...
			if dstCheck <= 10.0 then
				...
				if dstCheck <= 4.2 then
					ESX.Game.Utils.DrawText3D(SellPos, "Press ~g~[E]~w~ to Open Menu", 0.9)
			...
			for i = 1, #Config.VehiclePositions, 1 do
				if Config.VehiclePositions[i]["entityId"] ~= nil then
					...
					if dstCheck <= 2.0 then
						...
						ESX.Game.Utils.DrawText3D(vehCoords, "Press [E] to buy this car for: ~g~$" .. Config.VehiclePositions[i]["price"] .. ",-", 0.8)
...
function OpenSellMenu(veh, price, buyVehicle, owner)
	...
	if not buyVehicle then
		...
		table.insert(elements, { ["label"] = "Buy for $" .. price .. "", ["value"] = "buy" })
	...
	function(data, menu)
		...
		elseif action == "sell" then
			...
			ESX.TriggerServerCallback("esx-qalle-sellvehicles:isVehicleValid", function(valid)
				...
				if valid then
					DeleteVehicle(veh)
					ESX.ShowNotification("You put out the ~g~vehicle~s~ for sale ~g~$" .. price .. "")
					menu.close()
				else
		...
		elseif action == "buy" then
			ESX.TriggerServerCallback("esx-qalle-sellvehicles:buyVehicle", function(isPurchasable, totalMoney)
				if isPurchasable then
					DeleteVehicle(veh)
					ESX.ShowNotification("You ~g~bought~s~ the vehicle for " .. price .. ", your car is delivered to your garage")
					menu.close()
				else

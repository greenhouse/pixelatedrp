          ...
          local playerPed = PlayerPedId()
        	local job = PlayerData.job.name
                    ...
                    if job ~= 'police' or job ~= 'ambulance' then
                      local notification = 'Plate: ~y~' .. numberPlate .. '~w~ went ~r~' .. kmhSpeed .. '~w~ where ~g~' .. speedCam.maxSpeed .. '~w~ is allowed. You payed: ~g~$' ..fine
                      TriggerServerEvent('esx_mods:FinePlayer', GetPlayerServerId(PlayerId()) , fine , notification )
                    end
									end)
								end
							end

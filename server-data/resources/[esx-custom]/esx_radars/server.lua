	...
	MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE @plate = plate', {
		...
		if result[1] ~= nil then
			...
			MySQL.Async.execute('INSERT INTO billing (identifier, sender, target_type, target, label, amount) VALUES (@identifier, @sender, @target_type, @target, @label, @amount)',
			{
				...
				['@label']       = ("Plate %s, %s km/h indead of %s"):format(plate, kmhSpeed, maxSpeed),
				['@amount']      = amount
		...
		then
			...
			MySQL.Async.execute('INSERT INTO billing (identifier, sender, target_type, target, label, amount) VALUES (@identifier, @sender, @target_type, @target, @label, @amount)',
			{
				...
				['@target_type'] = 'society',
				['@target']      = 'society_police',
				['@label']       = "Company plate "..plate..", "..kmhSpeed.."km/h instead of "..maxSpeed,
				['@amount']      = amount
			}, function(rowsChanged)
				TriggerClientEvent('esx:showNotification', _source, "Your company car was flashed.")
			end)
		end
	end)
end)
...
ESX.RegisterUsableItem('coyotte', function(source)
	...
	if not IsEnabled then
		IsEnabled  = true
		TriggerClientEvent('esx_jb_radars:ShowRadarBlip', source)
		TriggerClientEvent('esx:ShowNotification',source, "You have activated your coyote.")
	else
		TriggerClientEvent('esx_jb_radars:RemoveRadarBlip', source)
		IsEnabled = false
	end

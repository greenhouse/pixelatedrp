...
AddEventHandler('esx_rallyjob:onRecord', function(track, laptime)
	...
	if not found then
		...
		TriggerClientEvent('esx:showNotification', _source, '~g~NEW! ~w~Track Record ~g~+~w~950~g~€ ~w~bonus!')
	...
	elseif found and record then
		table.insert(RallyTimes, {id = -1 , track = _track, driver = _driver, laptime = _laptime})
		xPlayer.addMoney(1700)
		TriggerClientEvent('esx:showNotification', _source, '~g~You broke the record! ~g~+~w~1700~g~€ ~w~bonus!')
		...
		TriggerClientEvent('esx:showNotification', _source, 'Unfortunately, you  ~r~did not ~w~break the track record!')
	end
end)
...
RegisterServerEvent('esx_rallyjob:PayPenalty')
AddEventHandler('esx_rallyjob:PayPenalty', function(amount)
	local xPlayer = ESX.GetPlayerFromId(source)

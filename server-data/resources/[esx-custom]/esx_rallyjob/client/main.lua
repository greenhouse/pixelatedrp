...
function refreshBlips()
	if playerData.job.name ~= nil and playerData.job.name == 'rally' then
		drawBlip(Config.locker, 366, "Rally Locker")
		drawBlip(Config.carSpawn, 315, "Take Rally car")
		drawBlip(Config.carDel, 490, "Return Rally car")
	end
end
...
function spawnCar()
	Citizen.CreateThread(function()
		...
		SetVehicleNumberPlateText(currentCar, "GUY" .. math.random(1000, 9999))
...
function startWork(flag)
	...
	if IsPedInAnyVehicle(playerPed) and isMyCar() then
		...
		if elementAt(taskPoints, currentIndex) == nil then
			ESX.ShowNotification('~w~Time to beat ~b~' .. lastRoute .. ' ~w~you got it ~y~' .. msToClock(stopWatch:SaveTime('lap')))
			...
			TriggerEvent('esx:showNotification', "Go back  ~b~to the garage.")
		else
			if flag then
				ESX.ShowNotification('~w~Interval: ~b~' .. msToClock(stopWatch:GetTime('lap')))
			end
			setGps(elementAt(taskPoints, currentIndex))
		end
	else
		ESX.ShowNotification('~r~Are you in a car?')
	end
end
...
function giveWork()
	...
	spawnCar()
	--ESX.ShowNotification("Deposit ~r~" .. Config.caution .. "~g~ € ~w~was taken from you.")
	--TriggerServerEvent('esx_rallyjob:getPunished', Config.caution)
	setGps(elementAt(taskPoints, currentIndex))
	ESX.ShowNotification("~w~Go to the ~b~the starting line.") --alert player
end
...
function getPayAmount()
	...
	if stopWatch:GetSavedTime('lap') < trackinfo.gold then
		...
		ESX.ShowNotification('~w~You got a ~y~Gold medal!')
	...
	elseif stopWatch:GetSavedTime('lap') < trackinfo.silver then
		...
		ESX.ShowNotification('~w~You got a ~y~Silver medal!')
		TriggerEvent('esx_rallyjob:celebrate')
	elseif stopWatch:GetSavedTime('lap') < trackinfo.bronze then
		...
		ESX.ShowNotification('~w~You got a ~y~Bronze medal!')
		TriggerEvent('esx_rallyjob:celebrate')
	else
	...
	ESX.ShowNotification('~w~You earned ~b~' .. amount .. '~g~ €~w~, Includes repair costs ~r~' .. penalty .. '~g~ €')
...
function getPaid()
	...
	elseif isMyCar() then
		deleteCar()
		TriggerServerEvent('esx_rallyjob:PayPenalty', 500)
		TriggerEvent('esx_rallyjob:ragequit')
		ESX.ShowNotification('~r~No rally, no pay. ~w~An entry fee was deducted from the panties, -~r~500~g~€')
	else
		TriggerEvent('esx_rallyjob:ragequit')
		ESX.ShowNotification('Whats wrong ~g~salty?')
		emeDel()
	end
	currentPlate = ''
	currentVehicle = nil
...
function getTrackinfo(lastRoute)
	...
	return {bronze = 993700, silver = 990800, gold = 810099, driver = "SneakGaming"} --default time should be quite high for auto-mode
end
	...
	while true do
		if onDuty and elementAt(taskPoints, currentIndex) ~= nil then
			...
			if currentIndex == 1 and isInside(pCoords, elementAt(taskPoints, currentIndex), 0.9) and isMyCar() then
				...
				FreezeEntityPosition(GetVehiclePedIsIn(GetPlayerPed(-1), false), true)
				ESX.ShowNotification('~w~The rally begins~w~...')
				startWork(false)

				ESX.TriggerServerCallback('esx_rallyjob:getTimes', function(times)
					RallyTimes = times
					local trackinfo = getTrackinfo(lastRoute)
					chat("7", "Route ^5::[ ^9" .. Config.Routes[lastRoute].name .. "^5 ]:: ^4bronze: ^3" .. msToClock(trackinfo.bronze) .. " ^0|^4 Silver: ^3" .. msToClock(trackinfo.silver) .. " ^0| ^4Gold: ^3" .. msToClock(trackinfo.gold))
					chat("7", "Gold is driving ^3@^9" .. trackinfo.driver)
					Citizen.Wait(3000)
...
function openMenu()
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'locker',
    {
      ...
      elements = {
        {label = "Rally clothes", value = 'rally_wear'},
        {label = "Casual clothing", value = 'everyday_wear'}
      }
    },
    function(data, menu)

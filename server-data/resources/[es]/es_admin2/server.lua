...
AddEventHandler('playerConnecting', function(user, set)
	for k,v in ipairs(GetPlayerIdentifiers(source))do
		if isBanned(v) then
			set(GetConvar("es_admin_banreason", "Du wurdest gebannt bitte melde dich im Support [discord.me/2ndlife]"))
			CancelEvent()
			break
		end
	end
			...
			TriggerEvent('es:canGroupTarget', user.getGroup(), groupsRequired[type], function(available)
				TriggerEvent('es:canGroupTarget', user.getGroup(), target.getGroup(), function(canTarget)
					if canTarget and available then
						...
						if type == "slay" then TriggerClientEvent('es_admin:quick', id, type) end
						if type == "kick" then DropPlayer(id, 'Du wurdest gekickt [discord.me/2ndlife]') end

						if type == "ban" then
							for k,v in ipairs(GetPlayerIdentifiers(id))do
								banUser(v)
							end
							DropPlayer(id, GetConvar("es_admin_banreason", "Du wurdest gebannt, bitte melde dich im Support [discord.me/2ndlife]"))
						end
					else
	...
	if args[1] then
		if(tonumber(args[1]) and GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				if(#reason == 0)then
					reason = "Kicked: Du wurdest Gekickt [discord.me/2ndlife]s"
				else
